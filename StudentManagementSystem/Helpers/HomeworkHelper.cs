...
public class HomeworkHelper
{
    ...
    public void CreateHomework(long teacherTckn)
    {
        ...
        var due = DateTime.Parse(Helper.Ask("Son teslim tarihi (gg.aa.yyyy)", true));
    ...
    public void UpdateHomework(long teacherTckn)
    {
        ...
        string newDateInput = Helper.Ask("Yeni teslim tarihi (gg.aa.yyyy, boş = aynı)");

        if (!string.IsNullOrWhiteSpace(newTitle)) selected.Task = newTitle;
        if (!string.IsNullOrWhiteSpace(newDesc)) selected.Description = newDesc;
        if (DateTime.TryParse(newDateInput, out DateTime parsedDate)) selected.EndDate = parsedDate;
